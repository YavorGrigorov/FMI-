...
/// I za chii se suglasih

dist_t getDistToClosestObjective(const Point& pt, const std::vector<Point>& objectives, size_t& objIdx) {
	if (!map.inBounds(pt)) return = MAX_DIST;
	dist_t minD = MAX_DIST;
	for (objIdx = 0; i < objectives.size(); ++objIdx) {
		dist_t t = calcDist(pt, objectives[objIdx]);
		if (t < minD) minD = t;
	}
	return minD;
}

static bool sameColor(const Point& lhs, const Point& rhs, const SourceArray& map) {
		return (sameColor(map.getPtColor(lhs.x, lhs.y), map.getPtColor(rhs.x, rhs.y))) 
}

static bool partOfObjective(const Point& pt, const std::vector<Point>& objectives, const SourceArray& map, size_t& ) {
	for (size_t i = 0; i < objectives.size(); ++i)
		if (sameColor(pt, objectives[i], map)) return true;
	return false;
}

static int np::setPathFromTo2(const Point&from, const std::vector<Point> objectives, SourceArray& map, const std::vector<color_t>& unpassable) {
	//Validation
	if (!map.inBounds(from)) return 1; // Have faith for the objectives
	LOG_TEXT("  Starting A*\n");
	...
	size_t objIdx = 0;
	toScan.push(std::make_pair(from, getDistToClosestObjective(from, objectives, objIdx)));
	...
	while (!toScan.empty()) {
		...
		else if (sameColor(map.getPtColor(curr.first.x, curr.first.y), map.getPtColor(to.x, to.y))) {
...
Path tryToFill(SourceArray& map, const Point& from, const std::vector<Point>& objectives, const std::vector<color_t>& unpassable) {

}

Path solve3(SourceArray& map) {
	std::vector<color_t> unpassable;
	ObjContainer objects = fillSourceArrayFromStart(map, unpassable);
	...
	for (size_t i = 1; i < objects.size(); ++i) {
		if		(objects[i].second == Key)  keys.push_back( objects[i].first);
		else if (objects[i].second == Exit) exits.push_back(objects[i].first);
		else if (objects[i].second == Door) doors.push_back(objects[i].first);
	}

	Point start = objects[0];
	...
	for (size_t i = 0; i < doors.size(); ++i) {
		bool kill = true;
		for (size_t j = 0; j < keys.size(); ++j)
			if (sameColor(doors[i], keys[j])) kill = false;
		if (!kill) unpassable.push_back(doors[i]);
	}
